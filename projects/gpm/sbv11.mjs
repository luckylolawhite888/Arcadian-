// Scarlett Bridge v11 — more engaging, always ends with a question or hook
...
server.listen(3001, () => console.log("SBv11 on 3001"));
...
async function handle(m) {
  ...
  try {
    ...
    // Ensure reply ends with a question or hook if it doesn't already
    const trimmed = reply.trim();
    const lastChar = trimmed.slice(-1);
    if (!trimmed.endsWith("?") && !trimmed.endsWith("?") && lastChar !== "?" && trimmed.length > 10) {
      reply = trimmed + " What do you think?";
    }
