...
async function run(){const count=parseInt(process.argv.find(a=>a.startsWith('--count='))?.split('=')[1]||'1',10);const seed=parseInt(process.argv.find(a=>a.startsWith('--seed='))?.split('=')[1]||String(Date.now()),10);console.log('GPREG v13 -',count,'submission(s)');const res=[];for(let i=0;i<count;i++){if(i>0)await slp(10000);res.push(await sub(seed+i));}console.log('\\nOK',res.filter(r=>r.gpref).length+'/'+count);res.forEach(r=>console.log(r.gpref?'  OK':'  FAIL',r.name,r.gpref||''));console.log('https://thenewworldorder.io/gpreg-tracker.html');}
async function go(p){await slp(500);for(const txt of['Continue','Submit','Save and continue','Accept and submit','Confirm and continue']){try{const el=p.locator('button').filter({hasText:txt}).first();if(await el.count({timeout:200}).catch(()=>0)>0){await el.click({timeout:1000}).catch(()=>{});await slp(500);return true;}}catch(e){}}try{const b=p.locator('button[type=submit]').first();if(await b.count({timeout:200}).catch(()=>0)>0){await b.click({timeout:1000}).catch(()=>{});await slp(500);return true;}}catch(e){}return false;}
async function fw(p){try{const inp=p.locator('button[type=submit]').first();await inp.click({timeout:1000}).catch(()=>{});await slp(1000);return true;}catch(e){return false;}}
...
async function fillt(p,n,v){try{const inp=p.locator('input[type="tel"][name="'+n+'"]').first();if(await inp.count({timeout:300})>0)await inp.pressSequentially(''+v,{delay:30,timeout:2000});}catch(e){}}
...
while(pages++<300){
await slp(800);await p.waitForLoadState('domcontentloaded').catch(()=>{});await slp(200);
...
else if(pn.includes('contact-details')&&pn.includes('current')){await fill(p,'phone',P.phone);skipRad=true;}
else if(pn.includes('contact-details')&&pn.includes('emergency')){await fillt(p,'emergencyContactPhone',P.phone);}
...
else if(pn.includes('previous-address')){await fill(p,'previousPostcode',P.postcode);await fill(p,'previousHouseNumber',P.houseNumber);}
else if(pn.includes('emergency-contact-detail')){await fill(p,'emergencyContactName',P.ecName);await fill(p,'emergencyContactRelationship',P.ecRelation);await ck(p);skipCk=true;}
...
await slp(2000);
try{const txt=await p.evaluate(()=>document.body.innerText);const m=txt.match(/GPREG-(\\d+-\\d+)/);if(m){gpref='GPREG-'+m[1];st='done';console.log('  OK',gpref);}else console.log('  FAIL no ref');}catch(e){console.log('  FAIL read err');}
