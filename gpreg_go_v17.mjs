...
async function run(){const count=parseInt(process.argv.find(a=>a.startsWith('--count='))?.split('=')[1]||'1',10);const seed=parseInt(process.argv.find(a=>a.startsWith('--seed='))?.split('=')[1]||String(Date.now()),10);console.log('GPREG v17 -',count,'submission(s)');const res=[];for(let i=0;i<count;i++){if(i>0)await slp(15000);res.push(await sub(seed+i));}console.log('\\nOK',res.filter(r=>r.gpref).length+'/'+count);res.forEach(r=>console.log(r.gpref?'  OK':'  FAIL',r.name,r.gpref||''));console.log('https://thenewworldorder.io/gpreg-tracker.html');}
...
else if(pn.includes('find-previous-address')||pn.includes('select-previous-address')){}
...
else if(pn.includes('vaccinations')){await rad(p,1);skipRad=true;}
else if(pn.includes('medical-condition')){await rad(p,1);skipRad=true;}
else if(pn.includes('allergies')){await rad(p,1);skipRad=true;}
else if(pn.includes('mental-health')){await rad(p,1);skipRad=true;}
else if(pn.includes('disabilities')||pn==='disability'){await rad(p,1);skipRad=true;}
else if(pn.includes('carer')){await rad(p,1);skipRad=true;}
else if(pn.includes('accessible-format')){await rad(p,1);skipRad=true;}
else if(pn.includes('reasonable-adjustments')){await rad(p,1);skipRad=true;}
else if(pn.includes('prescription')){await rad(p,1);skipRad=true;}
...
else if(pn.includes('alcohol-per-year')){await rad(p,0);skipRad=true;}
else if(pn.includes('alcohol')){await rad(p,0);skipRad=true;}
else if(pn.includes('smoke')){await rad(p,1);skipRad=true;}
...
await slp(2000);
try{const txt=await p.evaluate(()=>document.body.innerText);const ref=txt.match(/GPREG-[\d-]+/);if(ref){gpref=ref[0];st='done';console.log('  OK',gpref);}else console.log('  FAIL no ref');}
