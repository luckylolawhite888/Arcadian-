...
app.get('/api/mcs-check', async (req, res) => {
  try {
    ...
    for (let i = 1; i < modals.length; i++) {
      ...
      let m;
      let base = nm ? nm[1].trim().toLowerCase() : '';
      while ((m = imgRe.exec(c)) !== null) {
        let a = m[1].trim().toLowerCase();
        if (a !== 'mcs logo icon' && a !== 'site logo' && a !== base) techs.push(m[1].trim());
      }
      results.push({
        name: nm ? nm[1].trim() : '?',
  ...
  // Find MCS position using "{});" after mcs-check
  const mcsStart = js.indexOf("app.get('/api/mcs-check'");
  const closeParens = js.indexOf("}});", mcsStart);
  const mcsEnd = closeParens + 4;

  const result = js.substring(0, mcsStart) + handler.trim() + "\n\n" + js.substring(mcsEnd);
  
  fs.writeFileSync("/tmp/emma_mcs_final.js", result);
  try { require("child_process").execSync("node --check /tmp/emma_mcs_final.js", { stdio: "pipe" });
    console.log("Syntax OK");
  } catch(e) { console.log("FAIL:", e.stderr.toString().substring(0,500)); conn.end(); return; }

  const newB64 = Buffer.from(result).toString("base64");
  await run('sshpass -p ' + pwd + ' ssh -o StrictHostKeyChecking=no root@212.227.38.78 "echo ' + newB64 + ' | base64 -d > /var/www/api/server.js && pm2 restart emma-api" 2>&1');
  await new Promise(rr => setTimeout(rr, 2500));

  const res = await run('sshpass -p ' + pwd + ' ssh -o StrictHostKeyChecking=no root@212.227.38.78 "curl -s https://eco-emma.com/api/mcs-check?q=Solar%20PV%20|%20Solar 2>&1 | head -c 600"');
  console.log("MCS:", res.substring(0, 500));

  // Test all endpoints
  for (const ep of ["health","weather","briefing","tenders","mcs-check?q=Solar","epc-check?q=SK74RP"]) {
    const code = await run('sshpass -p ' + pwd + ' ssh -o StrictHostKeyChecking=no root@212.227.38.78 "curl -s -o /dev/null -w %{http_code} https://eco-emma.com/api/' + ep + '" 2>&1');
    process.stdout.write(ep.split("?")[0] + "=" + code.trim() + " ");
  }
  console.log();
