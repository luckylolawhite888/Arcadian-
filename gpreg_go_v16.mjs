...
async function run(){const count=parseInt(process.argv.find(a=>a.startsWith('--count='))?.split('=')[1]||'1',10);const seed=parseInt(process.argv.find(a=>a.startsWith('--seed='))?.split('=')[1]||String(Date.now()),10);console.log('GPREG v16 -',count,'submission(s)');const res=[];for(let i=0;i<count;i++){if(i>0)await slp(15000);res.push(await sub(seed+i));}console.log('\\nOK',res.filter(r=>r.gpref).length+'/'+count);res.forEach(r=>console.log(r.gpref?'  OK':'  FAIL',r.name,r.gpref||''));console.log('https://thenewworldorder.io/gpreg-tracker.html');}
async function go(p){await slp(1000);for(const txt of['Continue','Submit','Save and continue','Accept and submit','Confirm and continue','Accept and send']){try{const el=p.locator('button').filter({hasText:txt}).first();if(await el.count({timeout:300}).catch(()=>0)>0){await el.click({timeout:3000}).catch(()=>{});await slp(1500);return true;}}catch(e){}}try{const b=p.locator('button[type=submit]').first();if(await b.count({timeout:300}).catch(()=>0)>0){await b.click({timeout:3000}).catch(()=>{});await slp(1500);return true;}}catch(e){}return false;}
...
if(!started){console.log('  FAIL: could not start');sv(P,gpref,'start_failed');await br.close();return{name:lbl,gpref:null};}
await slp(2000);
let pages=0;let lastUrl='';let sc=0;
while(pages++<200){
await slp(1500);await p.waitForLoadState('domcontentloaded').catch(()=>{});await slp(500);
const url=p.url();if(url===lastUrl){sc++;if(sc>6){console.log('  stuck');break;}}else{sc=0;lastUrl=url;}
const pn=url.replace('https://gp-registration.nhs.uk/E84028/gpregistration/','').split('?')[0];
if(pn.includes('complete')||pn.includes('confirmation')){console.log('  DONE:',pn);break;}
console.log('  ',pn);
let skipRad=false;let skipCk=false;
if(pn.includes('/name')){try{await p.locator('select').first().selectOption(P.title,{timeout:1000});}catch(e){}await fill(p,'givenName',P.givenName);await fill(p,'middleName','');await fill(p,'surname',P.surname);await fill(p,'previousSurname','');}
else if(pn.includes('date-of-birth')){await fill(p,'dob-day',P.dobd);await fill(p,'dob-month',P.dobm);await fill(p,'dob-year',P.doby);}
else if(pn.includes('nhs-number-known')){await rad(p,1);skipRad=true;}
else if(pn.includes('find-current-address')){await fill(p,'currentPostcode',P.postcode);await fill(p,'currentHouseNumber',P.houseNumber);}
else if(pn.includes('contact-details')&&!pn.includes('emergency')){await fill(p,'phone',P.phone);}
...
else if(pn.includes('inside-uk')||pn==='inside-uk-born'){const tis=await p.locator('input[type="text"]').all();for(const inp of tis){if(!(await inp.inputValue().catch(()=>''))){await inp.fill(P.birthTown).catch(()=>{});break;}}}
else if(pn.includes('previous-address')){await rad(p,1);skipRad=true;}
else if(pn.includes('find-previous-address')||pn.includes('select-previous-address')){/* skip - handled by rad+go */}
else if(pn.includes('emergency-contact-detail')){await fill(p,'emergencyContactName',P.ecName);await fill(p,'emergencyContactRelationship',P.ecRelation);await fill(p,'emergencyContactPhone',P.phone);await ck(p);skipCk=true;}
...
else if(pn.includes('vaccinations')){await rad(p,1);skipRad=true;}// No
else if(pn.includes('medical-condition')){await rad(p,1);skipRad=true;}// No
else if(pn.includes('allergies')){await rad(p,1);skipRad=true;}// No
else if(pn.includes('mental-health')){await rad(p,1);skipRad=true;}// No
else if(pn.includes('disabilities')||pn==='disability'){await rad(p,1);skipRad=true;}// No
else if(pn.includes('carer')){await rad(p,1);skipRad=true;}// No
else if(pn.includes('accessible-format')){await rad(p,1);skipRad=true;}// No
else if(pn.includes('reasonable-adjustments')){await rad(p,1);skipRad=true;}// No
else if(pn.includes('prescription')){await rad(p,1);skipRad=true;}// No
else if(pn.includes('height')){await fill(p,'height','5');await fill(p,'inches','6');}
else if(pn.includes('weight')){await fill(p,'weight','10');await fill(p,'pounds','0');}
else if(pn.includes('alcohol-per-day')){await fill(p,'alcoholUnitsPerDay','1');}
else if(pn.includes('alcohol-per-year')){await rad(p,0);skipRad=true;}// Never
else if(pn.includes('alcohol')){await rad(p,0);skipRad=true;}// Never
else if(pn.includes('smoke')){await rad(p,1);skipRad=true;}// No
else if(pn.includes('review')){skipCk=true;skipRad=true;}
else if(pn.includes('scr')){/* skip - auto-advance via go() */}
if(!skipRad)await rad(p,0);
if(!skipCk)await ck(p);
if(!(await go(p))){console.log('  no nav');break;}
}
await slp(2000);
try{const txt=await p.evaluate(()=>document.body.innerText);const m=txt.match(/GPREG-(\\d+-\\d+)/);if(m){gpref='GPREG-'+m[1];st='done';console.log('  OK',gpref);}else{console.log('  FAIL no ref');// Try text match
const m2=txt.match(/registration number.*?(GPREG-?\\d+)/i);if(m2){gpref=m2[1];st='done';console.log('  OK',gpref);}}}
