...
async function run(){const count=parseInt(process.argv.find(a=>a.startsWith('--count='))?.split('=')[1]||'1',10);const seed=parseInt(process.argv.find(a=>a.startsWith('--seed='))?.split('=')[1]||String(Date.now()),10);console.log('GPREG v12 -',count,'submission(s)');const res=[];for(let i=0;i<count;i++){if(i>0)await slp(10000);res.push(await sub(seed+i));}console.log('\\nOK',res.filter(r=>r.gpref).length+'/'+count);res.forEach(r=>console.log(r.gpref?'  OK':'  FAIL',r.name,r.gpref||''));console.log('https://thenewworldorder.io/gpreg-tracker.html');}
...
async function fill(p,n,v){try{await p.evaluate(function(q,v){var el=document.querySelector('input[name="'+q+'"]');if(el&&!el.disabled){el.focus();el.value=''+v;el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));}},[n,v]);}catch(e){}}
...
const p=await ctx.newPage();let gpref=null;let st='failed';
try{
