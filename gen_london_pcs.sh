# Generate London postcodes from postcodes.io
python3 << 'PYEOF'
import urllib.request, json, random, sys, time

DISTRICTS = [
    "E1","E2","E3","E4","E5","E6","E7","E8","E9","E10","E11","E12","E13","E14","E15","E16","E17","E18","E20",
    "EC1","EC2","EC3","EC4",
    "N1","N2","N3","N4","N5","N6","N7","N8","N9","N10","N11","N12","N13","N14","N15","N16","N17","N18","N19","N20","N21","N22",
    "NW1","NW2","NW3","NW4","NW5","NW6","NW7","NW8","NW9","NW10","NW11",
    "SE1","SE2","SE3","SE4","SE5","SE6","SE7","SE8","SE9","SE10","SE11","SE12","SE13","SE14","SE15","SE16","SE17","SE18","SE19","SE20","SE21","SE22","SE23","SE24","SE25","SE26","SE27","SE28",
    "SW1A","SW1E","SW1H","SW1P","SW1V","SW1W","SW1X","SW1Y","SW2","SW3","SW4","SW5","SW6","SW7","SW8","SW9","SW10","SW11","SW12","SW13","SW14","SW15","SW16","SW17","SW18","SW19","SW20",
    "W1B","W1C","W1D","W1F","W1G","W1H","W1J","W1K","W1S","W1T","W1U","W1W","W2","W3","W4","W5","W6","W7","W8","W9","W10","W11","W12","W13","W14",
    "WC1","WC2",
    "BR1","BR2","BR3","BR4","BR5","BR6","BR7",
    "CR0","CR2","CR4","CR5","CR7","CR8",
    "DA1","DA5","DA6","DA7","DA8","DA14","DA15","DA16","DA17","DA18",
    "EN1","EN2","EN3","EN4","EN5","EN6","EN7",
    "HA0","HA1","HA2","HA3","HA4","HA5","HA6","HA7","HA8","HA9",
    "IG1","IG2","IG3","IG4","IG5","IG6","IG7","IG8","IG9","IG10","IG11",
    "KT1","KT2","KT3","KT4","KT5","KT6","KT7","KT8","KT9","KT10","KT11","KT12","KT13","KT14","KT15","KT16","KT17","KT18","KT19","KT20",
    "RM1","RM2","RM3","RM4","RM5","RM6","RM7","RM8","RM9","RM10","RM11","RM12","RM13","RM14","RM15","RM16","RM17","RM18","RM19","RM20",
    "SM1","SM2","SM3","SM4","SM5","SM6","SM7",
    "TW1","TW2","TW3","TW4","TW5","TW6","TW7","TW8","TW9","TW10","TW11","TW12","TW13","TW14",
    "UB1","UB2","UB3","UB4","UB5","UB6","UB7","UB8","UB9","UB10","UB11",
    "WD3","WD4","WD5","WD6","WD7","WD17","WD18","WD19","WD23","WD24","WD25",
]

random.shuffle(DISTRICTS)
postcodes = []
max_per_district = 3  # 3 per district = ~500 total

for d in DISTRICTS:
    if len(postcodes) >= 600:
        break
    for i in range(max_per_district * 2):  # try up to 6 times per district
        try:
            url = f"https://api.postcodes.io/random/postcodes/?outcode={d}"
            req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
            resp = urllib.request.urlopen(req, timeout=5)
            data = json.loads(resp.read())
            pc = data['result']['postcode']
            if pc not in postcodes:
                postcodes.append(pc)
                print(pc)
                sys.stdout.flush()
                break
            time.sleep(0.1)
        except Exception as e:
            time.sleep(0.2)
            continue

print(f"\n✅ Total: {len(postcodes)} postcodes", file=sys.stderr)
PYEOF
