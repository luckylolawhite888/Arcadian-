...
console.log('GPREG v10 -',count,'submission(s)');
...
// Start now - try every known selector
let started=false;
for(const sel of['#landing-start-button','button.landing-start__button','button:has-text("Start now")']){
if(started)break;
for(let t=0;t<3;t++){
try{await p.locator(sel).first().click({timeout:3000});started=true;break;}catch(e){}
await slp(500);
}}
...
else if(pn.includes('emergency-contact-detail')){
