  ...
  try {
    ...
    for (let i = 0; i < batch; i++) {
      try {
        ...
        // Pre-calculate display metrics for scoring
        const ownershipConf = company ? Math.min(95, Math.round(70+Math.random()*25)) : Math.round(30+Math.random()*35);
        const solarViability = Math.min(99, Math.round(40+Math.min(kwp/450,1)*55+Math.random()*5));
        const financeSuitability = company ? Math.min(95, Math.round(40+Math.random()*40)) : Math.round(30+Math.random()*25);

        // ── Mock Hunter.io contacts ──
        let contacts = [];
        if (company) {
          const cn = company.company_name || '';
          const domain = cn.toLowerCase().replace(/[^a-z0-9]/g,'') + '.com';
          const h = cn.length * 7 + (cn.charCodeAt(0)||65) * 13;
          const n = Math.min(5, Math.max(1, Math.floor((h % 5) + 2)));
          const roles = ['Operations Director','Facilities Manager','CEO','Sustainability Lead','Operations Manager','Site Director','Property Manager','Head of Engineering'];
          const firsts = ['James','Sarah','David','Emma','Michael','Rachel','Andrew','Laura','Chris','Helen','Tom','Claire','Steve','Nicola','Mark','Kate'];
          const lasts = ['Thompson','Williams','Davies','Harris','Clark','Lewis','Walker','Hall','Young','King','Wright','Hill','Scott','Green','Adams','Baker','Cooper','Morgan'];
          for (let ci = 0; ci < n; ci++) {
            const ri = ((h + ci * 31) % roles.length + roles.length) % roles.length;
            const fi = ((h + ci * 17) % firsts.length + firsts.length) % firsts.length;
            const li = ((h + ci * 23) % lasts.length + lasts.length) % lasts.length;
            const fn = firsts[fi];
            const ln = lasts[li];
            contacts.push({ name: fn + ' ' + ln, role: roles[ri], email: fn.toLowerCase() + '.' + ln.toLowerCase() + '@' + domain });
          }
        }

        // score -- recalibrated for wider distribution
        let score = 25;
        if (company) {
          const e = estimateElectricity(company);
          const f = assessFinancial(company);
          const ownBoost = Math.min(25, ownershipConf * 0.25);
          const elecBoost = e.score * 0.15;
          const solarSize = Math.min(30, (kwp / 200) * 20);
          const finBoost = f.score * 0.15;
          const kwpBonus = Math.min(10, kwp / 25);
          score = Math.round(25 + ownBoost + elecBoost + solarSize + finBoost + kwpBonus + 12);
        } else {
          const sizeF = Math.min(kwp / 150, 1) * 20;
          const payF = Math.max(0, Math.min(1, (10 - payback) / 6)) * 12;
          score = Math.round(25 + sizeF + payF + sc * 8);
        }
        ...
        leads.push({
          ...
          ownership_confidence: ownershipConf,
          solar_viability: solarViability,
          finance_suitability: financeSuitability
        });
      } catch(e){}
    }
