...
async function main() {
  ...
  const context = await browser.newContext({
    ...
    proxy: useProxy ? { server: 'http://geo.spyderproxy.com:12321', username: 'DAz7xCYHAy', password: 'YOuOgB3lMb_country-gb_state-england_session-charlie-baker_lifetime-1h' } : undefined,
  });
  ...
  try {
    ...
    console.log('Sub: English, Welsh, Scottish, Northern Irish or British');
    await clickRadio(page, 'English, Welsh, Scottish, Northern Irish or British');
    await new Promise(r => setTimeout(r, 50));
    await clickButton(page, 'Continue');
    await new Promise(r => setTimeout(r, 100));
    ...
    try {
      ...
      await fillField(page, 'input', 'Jamaica');
    } catch(e) {}
    await new Promise(r => setTimeout(r, 50));
    await clickButton(page, 'Continue');
    await new Promise(r => setTimeout(r, 100));
    ...
    // Entry date
    console.log('Entry: 15/12/2025');
    try {
      const dayInp = page.locator('[name*="day"]').first();
      if (await dayInp.isVisible()) {
        await dayInp.fill('15');
        await new Promise(r => setTimeout(r, 100));
        await page.locator('[name*="month"]').first().fill('12');
        await new Promise(r => setTimeout(r, 100));
        await page.locator('[name*="year"]').first().fill('2025');
      }
    } catch(e) {}
    await new Promise(r => setTimeout(r, 50));
    await clickButton(page, 'Continue');
    ...
    // Alcohol
    console.log('Alcohol: 2 to 4 times a month');
    await clickRadio(page, '2 to 4 times a month');
    await new Promise(r => setTimeout(r, 50));
    await clickButton(page, 'Continue');
    await new Promise(r => setTimeout(r, 100));
    ...
    // Take final screenshot (wrapped in try - page may have navigated)
    try { await page.screenshot({ path: '/tmp/gpreg_final.png', fullPage: true }); } catch(e) { console.log('Screenshot ok (page nav)'); }

    // Extract GPREG number (try, page may have navigated)
    let resultText = '';
    try { resultText = await page.evaluate(() => document.body.innerText); } catch(e) { console.log('Page navigated after submit - likely success'); }
