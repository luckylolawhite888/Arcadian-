...
async function run(){const count=parseInt(process.argv.find(a=>a.startsWith('--count='))?.split('=')[1]||'1',10);const seed=parseInt(process.argv.find(a=>a.startsWith('--seed='))?.split('=')[1]||String(Date.now()),10);console.log('GPREG v15 -',count,'submission(s)');const res=[];for(let i=0;i<count;i++){if(i>0)await slp(10000);res.push(await sub(seed+i));}console.log('\\nOK',res.filter(r=>r.gpref).length+'/'+count);res.forEach(r=>console.log(r.gpref?'  OK':'  FAIL',r.name,r.gpref||''));console.log('https://thenewworldorder.io/gpreg-tracker.html');}
async function go(p){await slp(1000);for(const txt of['Continue','Submit','Save and continue','Accept and submit','Confirm and continue']){try{const el=p.locator('button').filter({hasText:txt}).first();if(await el.count({timeout:300}).catch(()=>0)>0){await el.click({timeout:2000}).catch(()=>{});await slp(1500);return true;}}catch(e){}}try{const b=p.locator('button[type=submit]').first();if(await b.count({timeout:300}).catch(()=>0)>0){await b.click({timeout:2000}).catch(()=>{});await slp(1500);return true;}}catch(e){}return false;}
...
while(pages++<300){
await slp(2000);await p.waitForLoadState('domcontentloaded').catch(()=>{});await slp(500);
