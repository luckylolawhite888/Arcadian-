...
async function run(){const count=parseInt(process.argv.find(a=>a.startsWith('--count='))?.split('=')[1]||'1',10);const seed=parseInt(process.argv.find(a=>a.startsWith('--seed='))?.split('=')[1]||String(Date.now()),10);console.log('GPREG v14 -',count,'submission(s)');const res=[];for(let i=0;i<count;i++){if(i>0)await slp(10000);res.push(await sub(seed+i));}console.log('\\nOK',res.filter(r=>r.gpref).length+'/'+count);res.forEach(r=>console.log(r.gpref?'  OK':'  FAIL',r.name,r.gpref||''));console.log('https://thenewworldorder.io/gpreg-tracker.html');}
...
async function fill_pv(p,n,v){try{await p.evaluate(([n,v])=>{const el=document.querySelector('input[name="'+n+'"]');if(el&&!el.disabled){const t=el.type;el.focus();el.value=''+v;el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));el.blur();el.dispatchEvent(new Event('blur',{bubbles:true}));}},[n,v]);}catch(e){}}
...
while(pages++<300){
await slp(1200);await p.waitForLoadState('domcontentloaded').catch(()=>{});await slp(300);
const url=p.url();if(url===lastUrl){sc++;if(sc>4){console.log('  stuck');break;}}else{sc=0;lastUrl=url;}
...
else if(pn.includes('emergency-contact-detail')){await fill(p,'emergencyContactName',P.ecName);await fill(p,'emergencyContactRelationship',P.ecRelation);await fill_pv(p,'emergencyContactPhone',P.phone);await ck(p);skipCk=true;}
